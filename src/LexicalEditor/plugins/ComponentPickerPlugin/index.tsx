...
import { $createCodeNode } from '../../nodes/CodeNode/index.ts'
import {
...
import { INSERT_TABLE_COMMAND } from '../../nodes/TableNode/index.ts'
import {
  $createParagraphNode,
  $getSelection,
  $isRangeSelection,
...
function getBaseOptions(editor: LexicalEditor, showModal: ShowModal) {
  return [
    ...
    ...(['outdent', 'indent'] as const).map(
      (alignment) =>
        new ComponentPickerOption(`${alignment}`, {
          icon: <i className={`icon ${alignment}`} />,
          keywords: ['intend', 'outdent'],
          onSelect: () =>
            editor.dispatchCommand(INDENT_CONTENT_COMMAND, alignment),
        })
    ),
  ]
}
