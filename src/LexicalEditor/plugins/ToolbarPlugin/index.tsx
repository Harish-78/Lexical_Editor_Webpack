...
import { $isTableNode, $isTableSelection } from '../../nodes/TableNode/index.ts'
import {
  $findMatchingParent,
...
import { IS_APPLE } from '../../shared/environment.ts'
...
import { InsertDocumentDialog } from '../Document/DocumentPlugin.js'
import { InsertHTMLDialog } from '../HTMLPlugin/HtmlPlugin.js'
...
export default function ToolbarPlugin({
  ...
  setShowTableOfContents: Dispatch<boolean>
  htmlContent: any
  variables: any
  onChange: any
  ...
  const handleAutocompleteChange = (event: any, value: any) => {
    try {
      if (value) {
        editor.update(() => {
          const root = $getRoot()
          const textValue = value?.fullLabel
          ...
          if (isEmptyEditor) {
            ...
            paragraph.append($createTextNode(`{{${textValue}}}`))
            root.getFirstChild()?.replace(paragraph).select()
          } else {
            const selection = $getSelection()
            if (selection && $isRangeSelection(selection)) {
              selection.insertText(`{{${textValue}}}`)
            }
          }
        })
      }
  ...
  const renderDropDownItems = ({ componentName }) => {
    try {
  ...
  const defaultButtons = (
    <>
        ...
        <>
          ...
          {canViewerSeeInsertDropdown && (
            <>
              ...
              >
                {renderDropDownItems({ componentName })}
              </DropDown>
            </>
          )}
        </>
      ...
      >
        <i className="format html_renderer" />
      </button>
    </>
  )
  ...
  const attachmentButton = (
    <button
      disabled={!isEditable}
      onClick={() => {
        showModal('Insert Document', (onClose) => (
          <InsertDocumentDialog activeEditor={activeEditor} onClose={onClose} />
        ))
      }}
      className={'toolbar-item spaced '}
  ...
  const TableOfContentButton = (
    <button
      ...
      type="button"
    >
      <i className="format table_of_contents" />
    </button>
  )
  const renderToolbarButtons = ({ componentName }) => {
    try {
      switch (componentName) {
        case componentsEnum?.Wiki:
          return (
        ...
        case componentsEnum?.ManageTemplates:
          return (
            <>
              ...
              <Autocomplete
                id="to-autocomplete"
                options={variables?.length ? variables : []}
  ...
  return (
    <div className="toolbar">
      {renderToolbarButtons({ componentName })}
