...
function isTableSelectionRectangular(selection) {
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    if ($isTableCellNode(node)) {
      ...
      console.error("Expected CellNode to have a RowNode parent");
      if (currentRow !== row) {
...
function TableActionMenu({
  ...
  return createPortal(
    <div
      className="table_toolbar_dropdown"
      ref={dropDownRef}
      onClick={(e) => {
        e.stopPropagation();
      }}
    >
      ...
      >
        <ColorPicker
          ...
          showText={(color) => <span className="text">Bg-Color</span>}
          panelRender={(panel) => (
            <>
              {panel}
              {
                ...
                >
                  {" "}
                  <Button
                    disabled={!backgroundColor ? true : false}
                    size="small"
                    type="primary"
                    ...
                    onClick={() => {
                      handleCellBackgroundColor(backgroundColor);
                    }}
                  >
                    Apply
                  </Button>
